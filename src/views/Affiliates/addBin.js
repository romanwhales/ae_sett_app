...
import {fetchBinsPaginated} from '../../actions/bins';
import {addBinsToAffiliate} from '../../actions/affiliates'


import Pagination2 from "react-js-pagination";

class AddAffiliateBin extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedOption:null,
  ...
  clearNotifications =() => {
    ...
    this.props.bins.binAddedToAffiliate = null;
  }
  ...
  selectItem(e) {
    ...
    const { content } = this.props.bins.binList;
    const collection = [];
    const postArray =[];
    // this.props;
  ...
  addTerminals(){
    ...
    if (this.state.checkedListAll){
      ...
      this.props.addBinsToAffiliate(selectedids,this.props.match.params.id);
    }
      ...
      <div className="animated fadeIn">
        <Row>
          <Col>
           ...
           {this.props.bins.binAddedToAffiliate?<Alert color="success" isOpen={this.state.terminalAddedToTemplateAlert} toggle={this.onDismissTerminalAddedToTemplateSuccessAlert}>
                  Bins Added To Affiliate Successfully!
                </Alert>:null}
          ...
          {this.props.bins.binListError?<Alert color="danger" isOpen={this.state.terminalUpdatedErrorAlert} toggle={this.onDismissUpdatedTerminalErrorAlert}>
            {this.props.bins.binListError.response.data.message}
          </Alert>:null} 
            <Card>
              <CardHeader>
                <i className="icon-arrow-up"></i> Bins
              </CardHeader>
              <CardBody>
                <label>
                  <input
                ...
                <Table hover bordered striped responsive size="sm">
                  ...
                  <tbody>
                    {this.props.bins.binList
                        ? [
                            this.props.bins.binList.content.length > 0 ? this.props.bins.binList.content.map((item,index) => (
                                <tr key={index+1}>  
                                  <td>
                            ...
                            )):<tr key={1}><td colSpan="7"><p className="text-center text-danger">There are no bins at the moment.</p></td></tr>
                        ]
                        : [
                        <tr key={1}><td colSpan="7"><p className="text-center text-danger">There are no bins at the moment.</p></td></tr>
                        ]
                ...
                {/* {this.props.terminals.terminalList?<nav>
                <Pagination2
                    activePage={this.state.activePage}
                    itemsCountPerPage={20}
                    totalItemsCount={this.props.terminals.terminalList?this.props.terminals.terminalList.meta.totalElements:null}
...
export default connect(mapStateToProps,mapDispatchToProps)(AddAffiliateBin);