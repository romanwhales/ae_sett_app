...
class ProcessorSubGroupsProductsAddAccount extends Component{
    constructor(props){
        super(props);
        this.state = {
            subgroups:[],
    ...
    onSubmit = (values) =>{
        values.accounts.forEach((item)=>{
            item.allowManual = Boolean(item.allowManual);
        })
                ...
                <Row>
                    <Col xs="12">
                        {this.props.accountsCreated.accountCreatedError?<Alert color="danger" isOpen={this.state.createdAccountError} toggle={this.onDismissCreatedAccountErrorAlert}>
                            {this.props.accountsCreated.accountCreatedError.message}
                            </Alert>:null}
                    </Col>
                    <Col xs="12">
...
const mapStateToProps = (state) => {
  ...
  export default connect(mapStateToProps,mapDispatchToProps)(ProcessorSubGroupsProductsAddAccount);