...
import {fetchCards,createCard,fetchCard,updateCard} from '../../actions/cards';
import {fetchBinsPaginated,createBin,updateBin,fetchBin} from '../../actions/bins';
import {connect} from 'react-redux';
import {bindActionCreators} from 'redux';
import AddBinComponent from './addBin';
import UpdateBinComponent from './updateBin';
...
class Bins extends Component{
    ...
    local=''
    componentWillMount=()=>{
        this.props.fetchBinsPaginated(0)
    }
    ...
    handlePageChange = (pageNumber) => {
        ...
        this.props.fetchBinsPaginated(pageNumberParam);
        ...
        return(
            <div className="animated fadeIn">
                <Row>
                    ...
                    <Button style={{ 'marginBottom': '20px' }} color="success" onClick={this.toggle}><i className="fa fa-plus"></i>&nbsp; <FormattedMessage id="Add Bin" defaultMessage="Add Bin"/></Button>
                    </Col>
                </Row>
                <Row>
                    <Col>
                   {this.props.bins.binCreated?<Alert color="success" isOpen={this.state.createSuccessAlertVisible} toggle={this.onDismissCreateSuccessAlert}>
                    ...
                    {this.props.bins.binListError?<Alert color="danger" isOpen={this.state.updateProcessorErrorAlertVisible} toggle={this.onDismissUpdateProcessorErrorAlert}>
                        {this.props.bins.binListError.response.data.message}
                    </Alert>:null}
                    {/*{this.props.processorData.createProcessorError?<Alert color="danger" isOpen={this.state.createProcessorErrorAlertVisible} toggle={this.onDismisscreateProcessorErrorAlert}>
                        ...
                        <CardBody>
                            ...
                            <tbody>
                                ...
                                {  this.props.bins.binList
                                    ? [
                                        this.props.bins.binList.content.length > 0 ? this.props.bins.binList.content? this.props.bins.binList.content.map((item,index) => (
                                            <tr key={index+1}>
                                                <td>{index+1}</td>
                                        ...
                                        )):<tr><td colSpan="3"><p className="text-center text-danger"><FormattedMessage id="There are no bins created at the moment" defaultMessage="There are no bins created at the moment."/></p></td></tr>:<tr><td colSpan="4"><p className="text-center text-danger"><FormattedMessage id="There are no bins created at the moment" defaultMessage="There are no bins created at the moment."/></p></td></tr>
                                    ]
                                    : [
                                        ''
                                    ]
                                ...
                                {this.props.bins.binList?<nav>
                                <Pagination2
                                    activePage={this.state.activePage}
                                    itemsCountPerPage={20}
                                    totalItemsCount={this.props.bins.binList?this.props.bins.binList.meta.totalElements:null}
                      ...
                      <AddBinComponent {...this.state} {...this.toggle} onSubmit = {this.onSubmit} closeModal={this.closeModal} {...this.props}/>
                    <UpdateBinComponent {...this.state} {...this.updateToggle} closeUpdateModal ={this.closeUpdateModal} onSubmit={this.onSubmitUpdate} {...this.props}/>
            </div>
        )
    }
}
