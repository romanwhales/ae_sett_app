...
import {fetchInterAffiliateSummaryTransactions,downloadInterAffiliateSummaryTransactions} from '../../../actions/filesets';
...
class InterAffiliateIssuerSummaryTransactions extends Component {
    constructor(props){
        super(props);
        this.state ={
            modal:false,
  ...
  componentWillMount(){
      ...
      this.props.fetchInterAffiliateSummaryTransactions(this.props.match.params.date,0);
...
downloadInterAffiliateTransactions = (date) => {
  this.props.downloadInterAffiliateSummaryTransactions(date);
}
...
onSubmit = (values) => {
  ...
  render() {
    if(this.props.fileSets.interAffiliateSummaryTransactions){
      if(this.props.fileSets.interAffiliateSummaryTransactions.data.content.length > 0){
          delete this.props.fileSets.interAffiliateSummaryTransactions.data.content[0].interAffSummary;
          delete this.props.fileSets.interAffiliateSummaryTransactions.data.content[0].id;
          var columnHeaders = this.props.fileSets.interAffiliateSummaryTransactions.data.content[0];
          var newRows = [];
          ...
          var payloadArray = [];
          this.props.fileSets.interAffiliateSummaryTransactions.data.content.forEach(function(item){
              if(item.interAffSummary){
                  delete item.interAffSummary;
              }
              delete item.id
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
          <Col>
          {this.props.fileSets.interAffiliateSummaryTransactions?<CardGroup className="mb-4">
            <Widget04 icon="icon-people" color="info" header={this.props.fileSets.interAffiliateSummaryTransactions.data.totalElements.toLocaleString()} value="25">Transactions</Widget04>
          ...
          <Col lg={12}>
           {this.props.fileSets.fileSetFileDefinitionDeleted?<Alert color="success" isOpen={this.state.fileSetDeletedAlert} toggle={this.onDismissFileSetDeletedSuccessAlert}>
            ...
            <Card>
              ...
              <strong> {this.props.fileSets.interAffiliateSummaryTransactions ? <React.Fragment> 
                Inter Affilaite {this.props.match.params.date} Summary 
                </React.Fragment> : ""}<FormattedMessage id=" Transactions" defaultMessage=" Transactions"/></strong>
                {this.props.fileSets.interAffiliateSummaryTransactions ?[
                  this.props.fileSets.interAffiliateSummaryTransactions.data.content.length > 0 ? 
                  ...
                  <Button className={'float-right mb-0 btn-sm'} color="success" onClick={e => this.downloadInterAffiliateTransactions(this.props.match.params.date)}><i className="fa fa-download"></i>&nbsp; <FormattedMessage id="Download CSV" defaultMessage="Download CSV" /></Button>
                  :''
                ]:[
                  ''
                ]
              ...
              {   this.props.fileSets.interAffiliateSummaryTransactions
                                    ? [
                                        this.props.fileSets.interAffiliateSummaryTransactions.data.content.length > 0 ?  this.props.fileSets.interAffiliateSummaryTransactions? 
                                      <Grid
                                      rows={payloadArray}
                                      columns={newRows}
                                      >
                ...
                {this.props.fileSets.interAffiliateSummaryTransactions?<nav>
                    <Pagination2
                        activePage={this.state.activePage}
                        itemsCountPerPage={20}
                        totalItemsCount={this.props.fileSets.interAffiliateSummaryTransactions?this.props.fileSets.interAffiliateSummaryTransactions.data.totalElements:null}
...
const mapDispatchToProps= (dispatch) => {
  return bindActionCreators({
    fetchInterAffiliateSummaryTransactions,
    downloadInterAffiliateSummaryTransactions
  },dispatch)
}

export default connect(mapStateToProps,mapDispatchToProps)(InterAffiliateIssuerSummaryTransactions);
