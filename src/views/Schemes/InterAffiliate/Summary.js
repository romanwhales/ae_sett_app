...
import {fetchInterAffiliatePostingSummary,downloadInterAffiliatePostingSummary} from '../../../actions/filesets';
...
class InterAffiliateSummary extends Component {
    constructor(props){
        super(props);
        this.state ={
            modal:false,
    ...
    fileDefinitionId = ''
  componentDidMount(){
    this.props.fetchInterAffiliatePostingSummary(this.props.match.params.date);
  }
...
downloadInterAffiliateSummaryCsv = (date) => {
  this.props.downloadInterAffiliatePostingSummary(date);
}
          ...
          <Col lg={12}>
            <Card>
              ...
              {/* <strong> {this.props.fileSets.fileSetBatchDetail? <React.Fragment> 
                ...
                <strong>
                InterAffiliate Posting {this.props.match.params.date} Summary</strong>
                <div className="card-header-actions">
                {this.props.fileSets.interAffiliatePostingSummary ?[
                  this.props.fileSets.interAffiliatePostingSummary.data.length > 0 ? 
                  ...
                  <Button className={'float-right mb-0 btn-sm'} color="success" onClick={e => this.downloadInterAffiliateSummaryCsv(this.props.match.params.date)}><i className="fa fa-download"></i>&nbsp; <FormattedMessage id="Download CSV" defaultMessage="Download CSV" /></Button>
                  :''
                ]:[
                  ''
                ]
              ...
              <CardBody>
                <Table hover bordered striped responsive size="sm">
                    ...
                    {this.props.fileSets.interAffiliatePostingSummary
                        ? [
                            this.props.fileSets.interAffiliatePostingSummary.data.length > 0 ?   this.props.fileSets.interAffiliatePostingSummary.data.map((item,index) => (
                                    <React.Fragment>
                                    <tr >
                                    ...
                                    {item.records.map(record => (
                                        <React.Fragment>
                                            <tr>
                                                ...
                                                <td>{record.crDr}</td>
                                                <td>{record.settlementAmount >= 0? Number(record.settlementAmount).toLocaleString():'N/A'}</td>
...
const mapDispatchToProps= (dispatch) => {
  return bindActionCreators({
    fetchInterAffiliatePostingSummary,
    downloadInterAffiliatePostingSummary
  },dispatch)
}

export default connect(mapStateToProps,mapDispatchToProps)(InterAffiliateSummary);
