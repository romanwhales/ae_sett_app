...
import {fetchAcquirerTransactions,downloadInterAffiliateAcquirerCsv} from '../../../../actions/filesets';
...
class InterAffiliateAcquirerTransactions extends Component {
    constructor(props){
        super(props);
        this.state ={
            modal:false,
    ...
    fileDefinitionId = ''
  componentWillMount(){
      this.props.fetchAcquirerTransactions(this.props.match.params.date,0);
...
handlePageChange = (pageNumber) => {
  ...
  this.props.fetchAcquirerTransactions(this.props.match.params.date,pageNumberParam);
...
downloadTransactions = (date) => {
  this.props.downloadInterAffiliateAcquirerCsv(date);
}
...
onSubmit = (values) => {
  ...
  render() {
    if(this.props.fileSets.interAffiliateAcquirerTransactions){
      if(this.props.fileSets.interAffiliateAcquirerTransactions.data.content.length > 0){
          delete this.props.fileSets.interAffiliateAcquirerTransactions.data.content[0].interAffAcquirer;
          var columnHeaders = this.props.fileSets.interAffiliateAcquirerTransactions.data.content[0];
          var newRows = [];
          ...
          var payloadArray = [];
          this.props.fileSets.interAffiliateAcquirerTransactions.data.content.forEach(function(item){
              if(item.interAffAcquirer){
                  delete item.interAffAcquirer;
              }
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
          <Col>
          {this.props.fileSets.interAffiliateAcquirerTransactions?<CardGroup className="mb-4">
            <Widget04 icon="icon-people" color="info" header={this.props.fileSets.interAffiliateAcquirerTransactions.data.totalElements.toLocaleString()} value="25">Transactions</Widget04>
        ...
        <Row>
          ...
          <Col lg={12}>
           {this.props.fileSets.fileSetFileDefinitionDeleted?<Alert color="success" isOpen={this.state.fileSetDeletedAlert} toggle={this.onDismissFileSetDeletedSuccessAlert}>
             ...
             {this.props.fileSets.interAffiliateAcquirerTransactionsError?<Alert color="danger" isOpen={this.state.fileSetUpdatedAlert} toggle={this.onDismissFileSetUpdatedSuccessAlert}>
                    {this.props.fileSets.interAffiliateAcquirerTransactionsError.data.message}
                    </Alert>:null} 
            <Card>
              <CardHeader>
                
              <strong> {this.props.fileSets.interAffiliateAcquirerTransactions ? <React.Fragment> 
                InterAffiliate Acquirer {this.props.match.params.date}
                </React.Fragment> : ""}<FormattedMessage id=" Transactions" defaultMessage=" Transactions"/></strong>
                {this.props.fileSets.interAffiliateAcquirerTransactions ?[
                  this.props.fileSets.interAffiliateAcquirerTransactions.data.content.length > 0 ? 
                  ...
                  <Button className={'float-right mb-0 btn-sm'} color="success" onClick={e => this.downloadTransactions(this.props.match.params.date)}><i className="fa fa-download"></i>&nbsp; <FormattedMessage id="Download CSV" defaultMessage="Download CSV" /></Button>
                  :''
                ]:[
                  ''
                ]
              ...
              {   this.props.fileSets.interAffiliateAcquirerTransactions
                                    ? [
                                        this.props.fileSets.interAffiliateAcquirerTransactions.data.content.length > 0 ?  this.props.fileSets.interAffiliateAcquirerTransactions? 
                                      <Grid
                                      rows={payloadArray}
                                      columns={newRows}
                                      >
                ...
                {this.props.fileSets.interAffiliateAcquirerTransactions?<nav>
                    <Pagination2
                        activePage={this.state.activePage}
                        itemsCountPerPage={20}
                        totalItemsCount={this.props.fileSets.interAffiliateAcquirerTransactions?this.props.fileSets.interAffiliateAcquirerTransactions.data.totalElements:null}
...
const mapDispatchToProps= (dispatch) => {
  return bindActionCreators({
    fetchAcquirerTransactions,
    downloadInterAffiliateAcquirerCsv
  },dispatch)
}

export default connect(mapStateToProps,mapDispatchToProps)(InterAffiliateAcquirerTransactions);
