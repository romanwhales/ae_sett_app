...
import { Card, CardBody, CardHeader, Col, Row, Alert,CardGroup,Button} from 'reactstrap';
...
import {fetchT464Transactions,downloadT464Transactions} from '../../../../actions/filesets';
...
class MastercardT464Transactions extends Component {
    constructor(props){
        super(props);
        this.state ={
            modal:false,
    ...
    fileDefinitionId = ''
  componentWillMount(){
      this.props.fetchT464Transactions(this.props.match.params.fileId,0);
...
handlePageChange = (pageNumber) => {
  ...
  this.props.fetchT464Transactions(this.props.match.params.fileId,pageNumberParam);
...
downloadTransactions = (fileId) => {
  this.props.downloadT464Transactions(fileId);
}



  render() {
    if(this.props.fileSets.t464Transactions){
        if(this.props.fileSets.t464Transactions.data.content.length > 0){
            delete this.props.fileSets.t464Transactions.data.content[0].t464;
            var columnHeaders = this.props.fileSets.t464Transactions.data.content[0];
            var newRows = [];
            ...
            var payloadArray = [];
            this.props.fileSets.t464Transactions.data.content.forEach(function(item){
                if(item.t464){
                    delete item.t464;
                }
    ...
    // if(this.props.fileSets.fileSetDefinitionBatchesTranactionsPaginated){
    //   if(this.props.fileSets.fileSetDefinitionBatchesTranactionsPaginated.content.length > 0){
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
          <Col>
          {this.props.fileSets.t464Transactions?<CardGroup className="mb-4">
            <Widget04 icon="icon-people" color="info" header={this.props.fileSets.t464Transactions.data.meta.totalElements.toLocaleString()} value="25">Transactions</Widget04>
            ...
            <Widget04 icon="icon-pie-chart" color="primary" header="0" value="25">Debits</Widget04>
           ...
           {this.props.fileSets.fileSetFileDefinitionDeleted?<Alert color="success" isOpen={this.state.fileSetDeletedAlert} toggle={this.onDismissFileSetDeletedSuccessAlert}>
            ...
            <Card>
              <CardHeader>
                ...
                <strong> {this.props.fileSets.t464Transactions ? <React.Fragment> 
                Mastercard T464 {this.props.match.params.fileId}
                </React.Fragment> : ""}<FormattedMessage id=" Transactions" defaultMessage=" Transactions"/></strong>
                {this.props.fileSets.t464Transactions?[
                  this.props.fileSets.t464Transactions.data.content.length > 0 ? 
              ...
              <CardBody>
                {   this.props.fileSets.t464Transactions
                                    ? [
                                        this.props.fileSets.t464Transactions.data.content.length > 0 ?  this.props.fileSets.t464Transactions? 
                                      <Grid
                                      rows={payloadArray}
                                      columns={newRows}
                                      >
                ...
                {this.props.fileSets.t464Transactions?<nav>
                    <Pagination2
                        activePage={this.state.activePage}
                        itemsCountPerPage={20}
                        totalItemsCount={this.props.fileSets.t464Transactions?this.props.fileSets.t464Transactions.data.meta.totalElements:null}
...
const mapDispatchToProps= (dispatch) => {
  return bindActionCreators({
    fetchT464Transactions,
    downloadT464Transactions
  },dispatch)
}

export default connect(mapStateToProps,mapDispatchToProps)(MastercardT464Transactions);
