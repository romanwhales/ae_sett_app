...
import {fetchT112Summary,downloadT112PostingFile,fetchT112SummaryStats} from '../../../../actions/filesets';
...
class MastercardT112Summary extends Component {
    constructor(props){
        super(props);
        this.state ={
            modal:false,
  ...
  componentDidMount(){
    ...
    this.props.fetchT112Summary(this.props.match.params.fileId);
    this.props.fetchT112SummaryStats(this.props.match.params.fileId);
    debugger;
  }
...
downloadPostingFile = (fileId) => {
    this.props.downloadT112PostingFile(fileId);
}
  ...
  render() {
    ...
    return (
      <div className="animated fadeIn">
        ...
        {this.props.fileSets.t112SummaryStats?<Row>
          <Col>
          <CardGroup className="mb-4">
            <Widget05 icon="icon-people" color="info" header={this.props.fileSets.t112SummaryStats.data.settlementAmount.toLocaleString()} value="25" currencycode={affiliateHTML}>AMOUNT</Widget05>
            <Widget05 icon="icon-user-follow" color="success" header={this.props.fileSets.t112SummaryStats.data.creditCount.toLocaleString()} value="25" >CREDITS</Widget05>
            <Widget05 icon="icon-basket-loaded" color="warning" header={this.props.fileSets.t112SummaryStats.data.debitCount.toLocaleString()} value="25">DEBITS</Widget05>
        ...
        <Row>
          <Col lg={12}>
              {this.props.fileSets.t112SummaryError?<Alert color="danger" isOpen={this.state.fileSetT112ErrorAlert} toggle={this.onDismissFileSetT112ErrorAlert}>
                   {this.props.fileSets.t112SummaryError.data.message}
                    </Alert>:null} 
            <Card>
              <CardHeader>
              {/* <strong> {this.props.fileSets.fileSetBatchDetail? <React.Fragment> 
                ...
                <strong>
                Mastercard T112 {this.props.match.params.fileId} Summary</strong>
                {this.props.fileSets.t112Summary?[
                  this.props.fileSets.t112Summary.data.length > 0 ? 
              ...
              <CardBody>
                <Table hover bordered striped responsive size="sm">
                    ...
                    {this.props.fileSets.t112Summary
                        ? [
                            this.props.fileSets.t112Summary.data.length > 0 ?   this.props.fileSets.t112Summary.data.map((item,index) => (
                                    <React.Fragment>
                                    <tr >
                    ...
                    {this.props.fileSets.t112SummaryLoading? 
                      <Loader
                            type="ThreeDots"
                            color="#337AB7"
                            height={100}
...
const mapDispatchToProps= (dispatch) => {
  return bindActionCreators({
    downloadT112PostingFile,
    fetchT112Summary,
    fetchT112SummaryStats
  },dispatch)
}

export default connect(mapStateToProps,mapDispatchToProps)(MastercardT112Summary);
