...
import {fetchT140Transactions,downloadT140Transactions} from '../../../../actions/filesets';
...
class MastercardT140Transactions extends Component {
    constructor(props){
        super(props);
        this.state ={
            modal:false,
    ...
    fileDefinitionId = ''
  componentWillMount(){
      this.props.fetchT140Transactions(this.props.match.params.fileId,0);
  }
...
downloadTransactions = (fileId) => {
  this.props.downloadT140Transactions(fileId);
}
  ...
  render() {
    if(this.props.fileSets.t140Transactions){
      if(this.props.fileSets.t140Transactions.data.content.length > 0){
          delete this.props.fileSets.t140Transactions.data.content[0].t140;
          var columnHeaders = this.props.fileSets.t140Transactions.data.content[0];
          var newRows = [];
          for(var name in columnHeaders){
              var newObj = {};
              newObj.name = name;
              newObj.title = name.toUpperCase();
              newRows.push(newObj);
          }
          var payloadArray = [];
          this.props.fileSets.t140Transactions.data.content.forEach(function(item){
              if(item.t140){
                  delete item.t140;
              }
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
          <Col>
          {this.props.fileSets.t140Transactions?<CardGroup className="mb-4">
            <Widget04 icon="icon-people" color="info" header={this.props.fileSets.t140Transactions.data.meta.totalElements.toLocaleString()} value="25">Transactions</Widget04>
          ...
          <Col lg={12}>
           {this.props.fileSets.fileSetFileDefinitionDeleted?<Alert color="success" isOpen={this.state.fileSetDeletedAlert} toggle={this.onDismissFileSetDeletedSuccessAlert}>
            ...
            <Card>
              ...
              <strong> {this.props.fileSets.t140Transactions ? <React.Fragment> 
                Mastercard T140 {this.props.match.params.fileId}
                </React.Fragment> : ""}<FormattedMessage id=" Transactions" defaultMessage=" Transactions"/></strong>
                {this.props.fileSets.t140Transactions?[
                  this.props.fileSets.t140Transactions.data.content.length > 0 ? 
              ...
              {   this.props.fileSets.t140Transactions
                    ? [
                        this.props.fileSets.t140Transactions.data.content.length > 0 ?  this.props.fileSets.t140Transactions? 
                      <Grid
                      rows={payloadArray}
                      columns={newRows}
                      >
                ...
                {this.props.fileSets.t140TransactionsLoading? 
                <Loader
                      type="ThreeDots"
                      color="#337AB7"
                      height={100}
                  ...
                  />:''}
                {this.props.fileSets.t140Transactions?<nav>
                    <Pagination2
                        activePage={this.state.activePage}
                        itemsCountPerPage={20}
                        totalItemsCount={this.props.fileSets.t140Transactions?this.props.fileSets.t140Transactions.data.meta.totalElements:null}
...
const mapDispatchToProps= (dispatch) => {
  return bindActionCreators({
    fetchT140Transactions,
    downloadT140Transactions
  },dispatch)
}

export default connect(mapStateToProps,mapDispatchToProps)(MastercardT140Transactions);
