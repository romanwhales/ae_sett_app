...
import {fetchT461Transactions,downloadT461Transactions} from '../../../../actions/filesets';
...
class MastercardT461Transactions extends Component {
    constructor(props){
        super(props);
        this.state ={
            modal:false,
            ...
            fileSetT461TransactionError:true,
            deleteModal:false,
            activePage:1,
    ...
    fileDefinitionId = ''
  componentWillMount(){
      this.props.fetchT461Transactions(this.props.match.params.fileId,0);
  }
...
handlePageChange = (pageNumber) => {
  ...
  this.props.fetchT461Transactions(this.props.match.params.fileId,pageNumberParam);
...
onDismissFileSetT461ErrorAlert = () => {
  this.setState({
    fileSetT461TransactionError:false
  })
}

downloadTransactions = (fileId) => {
  this.props.downloadT461Transactions(fileId);
}
  ...
  render() {
    if(this.props.fileSets.t461Transactions){
      if(this.props.fileSets.t461Transactions.data.content.length > 0){
          delete this.props.fileSets.t461Transactions.data.content[0].t461;
          delete this.props.fileSets.t461Transactions.data.content[0].id;
          var columnHeaders = this.props.fileSets.t461Transactions.data.content[0];
          var newRows = [];
          ...
          var payloadArray = [];
          this.props.fileSets.t461Transactions.data.content.forEach(function(item){
              if(item.t461){
                  delete item.t461;
                  delete item.id;
              }
    ...
    return (
      <div className="animated fadeIn">
        ...
        <Row>
          <Col>
          {this.props.fileSets.t461Transactions?<CardGroup className="mb-4">
            <Widget04 icon="icon-people" color="info" header={this.props.fileSets.t461Transactions.data.meta.totalElements.toLocaleString()} value="25">Transactions</Widget04>
        ...
        <Row>
          ...
          <Col lg={12}>
           {this.props.fileSets.fileSetFileDefinitionDeleted?<Alert color="success" isOpen={this.state.fileSetDeletedAlert} toggle={this.onDismissFileSetDeletedSuccessAlert}>
             ...
             {this.props.fileSets.t461TransactionsError?<Alert color="danger" isOpen={this.state.fileSetT461TransactionError} toggle={this.onDismissFileSetT461ErrorAlert}>
                    {this.props.fileSets.t461TransactionsError.data.message}
            </Alert>:null} 
            <Card>
              <CardHeader>
                
              <strong> {this.props.fileSets.t461Transactions ? <React.Fragment> 
                Mastercard T461 {this.props.match.params.fileId}
                </React.Fragment> : ""}<FormattedMessage id=" Transactions" defaultMessage=" Transactions"/></strong>
                
                {this.props.fileSets.t461Transactions?[
                  this.props.fileSets.t461Transactions.data.content.length > 0 ? 
              ...
              {   this.props.fileSets.t461Transactions
                                    ? [
                                        this.props.fileSets.t461Transactions.data.content.length > 0 ?  this.props.fileSets.t461Transactions? 
                                      <Grid
                                      rows={payloadArray}
                                      columns={newRows}
                                      >
                ...
                {this.props.fileSets.t461Transactions?<nav>
                    <Pagination2
                        activePage={this.state.activePage}
                        itemsCountPerPage={20}
                        totalItemsCount={this.props.fileSets.t461Transactions?this.props.fileSets.t461Transactions.data.meta.totalElements:null}
...
const mapDispatchToProps= (dispatch) => {
  return bindActionCreators({
    fetchT461Transactions,
    downloadT461Transactions
  },dispatch)
}

export default connect(mapStateToProps,mapDispatchToProps)(MastercardT461Transactions);
